...
- A name is prose for a human, never an identifier. Never put an id or an id fragment in a name property: "arrival:fragile" and "early-checkin-rule" are ids, "tired arrival after a long journey" and "flexible early check-in" are names. When you emit two related vertices, name each for what it IS — never derive one vertex's name from the other's id, and never leave a name as a bare qualifier ("work", "fragile") when the concept is a guest type or a signal.
...
- When the expert corrects or updates ONE detail of a fact already captured ("actually it's three rooms now"), re-emit that fact with ONLY the corrected properties and its evidence. Properties you do not restate are preserved from the existing fact automatically — re-guessing unchanged fields from context corrupts them.
...
- Every edge endpoint id must be either (a) copied EXACTLY from KNOWN ENTITIES, or (b) the id of a vertex you emit in THIS delta. Never invent, guess, or placeholder an id ("servicefailure:undefined", a made-up hash, "x:session:hospitality:default" for a knowledge concept): if the relationship's other concept is asserted by the utterance but missing from the graph, emit that concept as its own vertex in this delta with its own evidence, then connect it.
- When the expert describes handling a problem, emit BOTH the ServiceFailure and its RecoveryAction and connect them with resolvedBy — a recovery without its failure is half the knowledge.
- GuestSignal is strictly an observable cue COMING FROM the guest (body language, pacing, booking behavior, requests, tone). Amenities, staff actions, or property features ("shaded courtyard", "refreshing drink offer") are NEVER GuestSignal — they are ServiceStandard or part of another concept.
- Statements about THIS INTERVIEW ITSELF — pacing preferences ("concise answers please"), consent, format requests — are session logistics, not hospitality knowledge. Emit nothing for them.
- ExpertRole and OperatingTenure describe the expert and their business: connect ExpertRole via hasRole from person:expert, and OperatingTenure via hasOperatingTenure from the HospitalityBusiness vertex (emit or reuse the business in the same delta) — never leave them floating.
