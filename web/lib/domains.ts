// The schema JSON in src/main/json/ is the single source of truth shared
// with the Python application (see "Schema: one source of truth" in
// CLAUDE.md). It is generated by `chatgraph-build-schema <domain>` and
// must never be hand-edited or copied into web/ -- the @schema/* alias
// (web/tsconfig.json) reads the committed artifacts in place.
import medicalSchemaRaw from "@schema/medical.json";
import hospitalitySchemaRaw from "@schema/hospitality.json";
...
// The committed schema JSON is generated by `chatgraph-build-schema` and
// uses `key`/`value`. Hydra's pre-0.17.1 encoding used `@key`/`@value`,
// and some hand-written artifacts still do, so both spellings are
// accepted here -- the same tolerance already applied to out/outV below.
// Readers must go through entryKey()/entryValue() rather than indexing
// either spelling directly.
