// medical.json is the canonical artifact shared with the Python application
// (src/main/json/, generated by `chatgraph-build-schema`); read it in place
// via the @schema/* alias rather than keeping a copy here.
import medicalSchemaRaw from "@schema/medical.json";
// hospitality.json is still local: the gate's schema (supersededBy, widened
// provenance domains, multi-label endpoints) has moved ahead of the committed
// root artifact. Reconciling the two is tracked in issue #1.
...
// `chatgraph-build-schema` emits `key`/`value`; Hydra's pre-0.17.1 encoding
// used `@key`/`@value`, and the hospitality artifact still does. Both are
// accepted, the same tolerance already applied to out/outV. Read entries
// through entryKey()/entryValue(), never by indexing one spelling.
export type DomainSchema = {
  vertices: Array<{
    key?: string;
    "@key"?: string;
    value?: { properties?: SchemaProps };
    "@value"?: { properties?: SchemaProps };
  }>;
  edges: Array<{
    key?: string;
...
/** Label of a schema entry, in either the `key` or legacy `@key` spelling. */
export function entryKey(entry: { key?: string; "@key"?: string }): string {
  return entry.key ?? entry["@key"] ?? "";
}

/** Body of a schema entry, in either the `value` or legacy `@value` spelling. */
export function entryValue<T>(entry: { value?: T; "@value"?: T }): T | undefined {
