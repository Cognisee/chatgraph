...
- `src/main/hydra/` — schema modules in Hydra's native textual syntax
  (`.hy`), namespaced reverse-DNS and organised hierarchically. **These
  are documentation, not executable**: the Hydra-to-PG encoding is
  expected in Hydra 0.18.1, and until then the authoritative schema is
  still the generated JSON below. Syntax conventions and known gaps in
  the upstream docs are in `docs/hydra-textual-syntax-notes.md`.
