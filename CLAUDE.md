...
projects (Hydra, the Hydra Python dist packages).
...
- `src/main/python/chatgraph/schema/pgdsl.py` — fluent builders and the
  canonical JSON encoder used by every domain's `schema_build.py`.
  Authoring-time only; the runtime never imports it.
- `src/test/python/chatgraph/` — smoke tests. Some call real Claude
  APIs and cost money per run; gated on the `ANTHROPIC_API_KEY` env var.
...
Hydra (`hydra-kernel`, `hydra-pg`) is a normal PyPI dependency pinned in
`pyproject.toml`; there is no source-level dependency and no env var to
set. chatgraph formerly depended on a local HydraPop clone via
`HYDRAPOP_HOME`; as of Hydra 0.17.1 everything it provided ships in
`hydra-pg` and its language-specific overlays, and that dependency is
gone.

Where the pieces live (all verified against 0.17.1):

- **Hydra ↔ TinkerPop bridge** — `hydra.overlay.python.tinkerpop.coder`
  (`hydra_to_gremlin`, `gremlin_to_hydra`), plus the validation helpers
  `Result` and `check_literal`. This is the `hydra.overlay.python.*`
  tree: the Python-specific overlay, i.e. hand-written host-language
  code as opposed to Hydra's generated modules.
- **Python DSL helpers** — `hydra.overlay.python.dsl.python`
  (`FrozenDict`, `Given`, `None_`) and
  `hydra.overlay.python.dsl.literal_types` (`string()`, `int32()`,
  `boolean()`, ...). These are zero-arg *functions* returning a
  `LiteralType`.
- **Executable coders** — `hydra.encode.*`, `hydra.decode.*`,
  `hydra.json.encode`, `hydra.json.writer`. Their `hydra.dsl.*`
  counterparts (e.g. `hydra.dsl.json.encode`) are term-level DSLs that
  build `TypedTerm`s rather than doing the work; picking the wrong one
  yields a `TypedTerm` where you expected a result.

Two API details that bite (both cost real debugging time):

1. **Hydra optionals are not Python optionals.** `hydra.validate.pg`'s
   `label_for_vertex_id` resolver must return `Given(x)` / `None_()`,
   and be passed as `Given(resolver)`. The type hints say
   `Optional[Callable[...]]`, but handing it a bare Python `None`
   makes `validate_edge`'s internal fold collapse to `None` and raise
   `AttributeError: 'NoneType' object has no attribute 'errors'` from
   deep inside generated code. (This replaced 0.15's `Just`/`Nothing`.)
2. **Validation returns findings, not a Maybe.** `validate_vertex` /
   `validate_edge` take a `ValidationProfile` first
   (`default_pg_profile()`; `max_errors=1` reproduces the old
   first-error-wins behaviour) and return a `ValidationResult` with
   `errors` / `warnings` ConsLists.

## Session procedure

1. **Identify the working directory.** Most edits happen under
   `src/main/python/chatgraph/`. Read the repo state with
